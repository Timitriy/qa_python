...
# ---------- add_new_book ----------
@pytest.mark.parametrize('title', ['Книга', 'A' * 40])  # 1 и 40 символов
...
@pytest.mark.parametrize('title', ['', 'A' * 41])       # 0 и 41 символ
...
def test_add_new_book_duplicate(collector):
    ...
    collector.add_new_book('Дюна')
    assert list(collector.books_genre).count('Дюна') == 1
...
def test_set_book_genre_invalid(collector):
    collector.add_new_book('Дюна')
    collector.set_book_genre('Дюна', 'Поэзия')          # жанра нет
...
# ---------- favorites ----------
def test_add_book_in_favorites_adds_unique_book(collector):
    collector.add_new_book('Дюна')
    collector.add_book_in_favorites('Дюна')
    collector.add_book_in_favorites('Дюна')  # повтор — должен игнорироваться
...
def test_get_list_of_favorites_books_returns_actual_list(collector):
    collector.add_new_book('Дюна')
